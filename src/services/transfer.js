...
module.exports = (app) => {
	...
	const validade = async (transfer) => {
		...
		if(!transfer.acc_ori_id) throw new ValidationError('Conta origem é um atributo obrigatório');
		if(!transfer.acc_dest_id) throw new ValidationError('Descrição é um atributo obrigatório');
		if(transfer.acc_ori_id === transfer.acc_dest_id) throw new ValidationError('Não é permitido que a contas de origem e destino seja iguais');
	...
	return { find, save, findOne, update, validade };
};